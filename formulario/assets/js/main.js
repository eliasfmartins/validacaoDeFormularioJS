...
class ValidaFormulario{
  ...
  senhasSaoValidas(){
    ...
    if(senha.value.length<6 || senha.value.length>12){
      valid = false;
      this.criaError(senha,'Senha precisa ter de 6 a 12 caracteres ')
    }
  ...
  camposSaoValidos(){
    ...
    for(let campo of this.formulario.querySelectorAll('.validar')){
      ...
      if(!campo.value){
        this.criaError(campo,`"${label}"  não pode ficar em branco `);
        valid = false;
      }
  ...
  validaUsuario(campo){
    ...
    if(!usuario.match(/^[a-zA-Z0-9]+$/g)) {
      this.criaError(campo,'Nome de usuário precisa conter apenas letras e números ')
      valid = false;
    }
